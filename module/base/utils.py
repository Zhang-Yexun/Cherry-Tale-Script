...
'''
图像处理和识别
load_image, save_image: 加载和保存图像文件。
crop: 裁剪图像。
resize: 改变图像的大小。
rgb2gray, rgb2hsv, rgb2yuv, rgb2luma: 将RGB图像转换为灰度图像、HSV色彩空间、YUV色彩空间、亮度(Luma)图。
get_color: 获取图像指定区域的平均颜色。
get_bbox, get_bbox_reversed: 获取图像中非白(黑)背景的内容的边界框。
extract_letters, extract_white_letters: 从图像中提取文字，将文字设为黑色，背景设为白色，用于文字识别。
color_similarity, color_similar: 计算两种颜色的相似度。
color_mapping: 对图像的颜色进行映射，使其分布在一个指定的范围内。
数学计算与模拟
random_normal_distribution_int: 生成在指定范围内，服从正态分布的随机整数。
random_rectangle_point: 在指定的矩形区域内随机选择一个点。
random_rectangle_vector, random_rectangle_vector_opted: 在指定的矩形框内，根据给定的向量随机放置该向量，并考虑白名单区域和黑名单区域。
random_line_segments: 将一条线分割成多个随机长度的线段。
ensure_time: 确保给定的时间值在一个特定的范围内。
坐标与区域处理
area_offset, area_pad, limit_in, area_limit, area_size, point_limit, point_in_area, area_in_area, area_cross_area: 
这些函数用于处理图像中的区域和坐标点，包括移动区域、缩放区域、限制点或区域在另一个区域内等。
其他辅助函数
float2str, point2str, col2name, name2col, node2location, location2node: 这些函数提供了格式化输出、列名与列索引之间的转换、节点名与位置之间的转换等辅助功能。
整体来说，这个工具库为处理图像、模拟随机事件以及图像识别等任务提供了强大的支持。
'''
