...
'''
这段代码定义了一个Page类，用于表示和管理游戏内不同页面之间的导航关系。通过这个类和其实例，
可以在游戏自动化脚本中模拟用户的页面切换操作，从而实现对游戏内多个功能和视图的访问和控制。

类属性和方法
all_pages: 类属性，用于存储所有页面实例的字典，键为页面名称（字符串），值为Page实例。
clear_connection: 类方法，清除所有页面的父级关系。
init_connection: 类方法，初始化页面之间的连接关系，基于目的页面建立一个导航路径。
iter_pages: 类方法，遍历所有页面实例。
iter_check_buttons: 类方法，遍历所有页面的检查按钮。
实例属性和方法
check_button: 实例属性，表示用于识别页面的按钮。
links: 实例属性，存储从当前页面到其他页面的导航链接，键为目的地Page实例，值为执行导航的按钮。
name: 实例属性，页面的名称。
parent: 实例属性，页面的父级页面，用于导航路径搜索。
link: 实例方法，用于链接当前页面到另一个页面。
页面定义和链接
代码中通过Page类的实例定义了游戏内的多个页面，例如page_main、page_campaign、page_event等，
并通过调用实例的link方法设置页面之间的导航链接。这样，当需要从一个页面跳转到另一个页面时，可以根据这些定义的链接来确定点击哪个按钮。
例如：

page_main.link(button=MAIN_GOTO_CAMPAIGN, destination=page_campaign_menu)：表示从主页面（page_main）
通过点击MAIN_GOTO_CAMPAIGN按钮可以导航到战役菜单页面（page_campaign_menu）。
导航和页面检测
这种页面和链接的定义方式使得在自动化脚本中可以根据当前页面的状态来决定下一步操作，
例如，检查特定的按钮是否出现来确定当前所在的页面，以及根据目标页面选择合适的按钮进行点击操作。
此外，通过init_connection方法和A*路径查找算法，可以优化在复杂页面结构中的导航路径，提高自动化脚本的效率和稳定性。'''
